...
class PACMod(object):
    ...
    def stanley_gnss_callback(self, msg):
        ...
        # added just in case [ comment out if unnecessary ]
        self.ackermann_msg_gnss.steering_angle_velocity = round(msg.steering_angle_velocity ,2)
        self.ackermann_msg_gnss.jerk                    = round(msg.jerk ,2)
      ...
      while not rospy.is_shutdown():
        ...
        if(self.pacmod_enable == True):
          ...
          if (self.gem_enable == False):
            ...
            curr_st_ang = self.steer
            curr_accel  = self.ackermann_msg_gnss.acceleration
            decisionModule = VehicleDecision()
            target_v, lateral_error, lane_theta = decisionModule.get_ref_state(curr_speed, front_dist, lateral_error, lane_theta)
            ...
            self.brake_cmd, throttle_percent, steering_angle = controlModule.execute(target_v, lateral_error, lane_theta)
