...
class ImageConverter:
  ...
  def perspective_transform(self, img, verbose=False):
    ...
    bot_right = [0,0]
    bot_left  = []
    GEM_e2_bag_src = np.float32([[450, 250], [0, 375], [1242, 375], [750, 250]])
    GEM_e2_bag_dst = np.float32([[0, 0]    , [0, 375], [1242, 375], [1242, 0]])
    # --------------- END TUNING --------------- 
  ...
  def detection(self, img):
    ...
    if not self.hist:
      ...
      # Annotate original image # [originally from gem_vision]
      # bird_fit_img = None
      ...
      if ret is not None:
        ...
        """ we can add velocity control... """
        # constants 
        ROWS = 720   # 0 -> 719 as you go down
        COLS = 1280  # 0 -> 1279 as you go right
        ROWS_MID = ROWS / 2
        COLS_MID = COLS / 2
        A_l, B_l, C_l = ret['left_fit']
        A_r, B_r, C_r = ret['right_fit']
        LANE_WIDTH_METERS = 1.2 # ???? TODO: MEASURE THIS

        """ UNCOMMENT for velocity control """
        # curr_v = 1     # get the subscriber
        # curr_sa = 1    # get the subscriber
        # time_step = 1  # get the position in pixels at time = t_curr + 1 (next time step). Hope i can do mulitple 
        # pos_at_next_time_step = curr_v * np.cos(curr_sa) * time_step
        # rows_from_bottom = pos_at_next_time_step * self.pixel_per_meter
        # rows_from_top = ROWS - rows_from_bottom
        # y = rows_from_top

        """ UNCOMMENT for non_velocity control """
        rows_from_bottom = (1) * self.pixel_per_meter 
        rows_from_top = ROWS - rows_from_bottom
        y = rows_from_top
        ...
        """ if it is unable to get a left or right fit line, then handle finding the  """
        if ret['left_fit'] is not None and ret['right_fit'] is not None:
          col_ave = (col_l + col_r) / 2
          dangle_ave = (dangle_l + dangle_r) / 2  
        
        elif ret['left_fit'] is not None and ret['right_fit'] is None: # we must determine/predict the col_r
          col_l = col_r - (LANE_WIDTH_METERS * self.pixel_per_meter)
          col_ave = (col_l + col_r) / 2
          dangle_ave = (dangle_l)
        
        elif ret['left_fit'] is None and ret['right_fit'] is not None: # we must determine/predict the col_l
          col_r = col_l + (LANE_WIDTH_METERS * self.pixel_per_meter)
          col_ave = (col_l + col_r) / 2
          dangle_ave = (dangle_l)


        # set lateral error
        lateral_error_pixels = COLS_MID - col_ave
        lateral_error_meters = lateral_error_pixels * self.meter_per_pixel

        lateral_error = lateral_error_meters
