...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		y_upper = window * window_height
		y_lower = (window + 1) * window_height
		leftx_left_side = leftx_current - margin
		leftx_right_side = leftx_current + margin
		rightx_left_side = rightx_current - margin
		rightx_right_side = rightx_current + margin
		####
		# Draw the windows on the visualization image using cv2.rectangle()
		cv2.rectangle(out_img, (leftx_left_side, y_upper), (leftx_right_side, y_lower), (0, 255, 0), -1)
		cv2.rectangle(out_img, (rightx_left_side, y_upper), (rightx_right_side, y_lower), (255, 0, 0), -1)
		####
		# Identify the nonzero pixels in x and y within the window
		y_nonzero_window = (nonzeroy < y_lower) & (nonzeroy >= y_upper)
		window_nonzero_left = (y_nonzero_window & (nonzerox >= leftx_left_side) & (nonzerox < leftx_right_side)).nonzero()[0]
		window_nonzero_right = (y_nonzero_window & (nonzerox >= rightx_left_side) & (nonzerox < rightx_right_side)).nonzero()[0]
		####
		...
		if len(window_nonzero_right) > minpix:
			rightx_new = np.int(np.mean(nonzerox[window_nonzero_right]))
			# if rightx_new < leftx_current:
			rightx_current = rightx_new
		if len(window_nonzero_left) > minpix:
			leftx_new = np.int(np.mean(nonzerox[window_nonzero_left]))
			# if leftx_new > rightx_current:
...
def tune_fit(binary_warped, left_fit, right_fit):
	...
	left_fit = np.polyfit(lefty, leftx, 2)         # Polynomial coefficients, highest power first. [Ay^2 + By + C]
	right_fit = np.polyfit(righty, rightx, 2)      # Polynomial coefficients, highest power first.
